 ...
 * {@link LimpezaFragment.OnFragmentInteractionListener} interface
 * to handle interaction events.
 * Use the {@link LimpezaFragment#newInstance} factory method to
 * create an instance of this fragment.
 */
public class LimpezaFragment extends Fragment {
    RadioGroup rgLimpa;
    RadioButton rbTodos, rbEnviado;
    Button btLimpa;
    ...
    private void setupComps(View v) {
        ...
        rbEnviado   = (RadioButton) v.findViewById(R.id.rbEnviados);
        rbTodos     = (RadioButton) v.findViewById(R.id.rbTodos);
        btLimpa     = (Button) v.findViewById(R.id.btLimpa);
        toast = new MyToast(PrincipalActivity.chagasContext, Toast.LENGTH_SHORT);
                ...
                switch (rgLimpa.getCheckedRadioButtonId()) {
                    case R.id.rbTodos:
                        new AlertDialog.Builder(getActivity())
                                ...
                                .setMessage("Essa opção irá excluir inclusive os insetos notificados não atendidos. Confirma essa ação?")
                                .setPositiveButton("Sim", new DialogInterface.OnClickListener()
                                {
                                    @Override
                                    public void onClick(DialogInterface dialog, int which) {
                                        Suspeito sus = new Suspeito(0);
                                        sus.limpar();
                                        Triatomineo.Limpar(getActivity(),"");
                                    }
                                })
                        ...
                        break;
                    default:
                        Triatomineo.Limpar(getActivity(),"WHERE status=1");
                        break;
                }
                toast.show("Registros excluídos.");
            }
        });
    }
